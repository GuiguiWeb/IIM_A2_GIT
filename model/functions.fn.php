...
function getArtistWithMusicName($musicName)
{
    $arr = explode(' ', trim($musicName));
    return $arr[0];
}

function getTagsByArtist($artist)
{
    $headers = array('Accept' => 'application/json');
    $response = Unirest\Request::get('http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&lang=fr&artist=' . $artist . '&api_key=49384a6c4639addf62ac64e8d0d5300f&format=json', $headers);
    if (property_exists($response->body, 'message')) {
        return $response->body->message;
    } elseif (property_exists($response->body, 'artist')) {
        $tags = (array)$response->body->artist->tags;
        return $tags['tag'];
    } else {
